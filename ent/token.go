...
type Token struct {
	...
	// CeratedAt holds the value of the "cerated_at" field.
	CeratedAt time.Time `json:"cerated_at,omitempty"`
...
func (*Token) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case token.FieldID, token.FieldUID:
			values[i] = new(sql.NullInt64)
		case token.FieldToken, token.FieldUserAgent:
			values[i] = new(sql.NullString)
		case token.FieldExpiresAt, token.FieldCeratedAt:
...
func (_m *Token) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case token.FieldCeratedAt:
			if value, ok := values[i].(*sql.NullTime); !ok {
				return fmt.Errorf("unexpected type %T for field cerated_at", values[i])
			} else if value.Valid {
				_m.CeratedAt = value.Time
			}
		default:
...
func (_m *Token) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("cerated_at=")
	builder.WriteString(_m.CeratedAt.Format(time.ANSIC))
	builder.WriteByte(')')
